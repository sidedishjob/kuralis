...
describe("LoginForm", () => {
  ...
  describe("レンダリング", () => {
    test("フォームの基本要素が表示される", () => {
      ...
      expect(
        screen.getByText("Googleアカウントまたはメールでログイン"),
      ).toBeInTheDocument();
      expect(
      ...
      expect(
        screen.getByRole("button", { name: "ゲストモードで試す" }),
      ).toBeInTheDocument();
    });
  ...
  describe("カスタムクラス", () => {
    test("カスタムクラスが適用される", () => {
      const { container } = render(
        <LoginFormComponent className="custom-class" />,
      );
    ...
    test("追加のpropsが適用される", () => {
      const { container } = render(
        <LoginFormComponent data-testid="login-form" />,
      );
