...
describe("SignupForm", () => {
	...
	describe("レンダリング", () => {
		test("フォームの基本要素が表示される", () => {
			...
			expect(screen.getByText("Googleまたはメールアドレスで登録")).toBeInTheDocument();
	...
	describe("エラーハンドリング", () => {
		test("認証エラー後、再度登録を試すとエラーメッセージがクリアされる", async () => {
			...
			await waitFor(() => {
				expect(screen.queryByText("もう一度お試しください")).not.toBeInTheDocument();
			});
	...
	describe("カスタムクラス", () => {
		test("カスタムクラスが適用される", () => {
			const { container } = render(<SignupFormComponent className="custom-class" />);
			expect(container.firstChild).toHaveClass("custom-class");
		});

		test("追加のpropsが適用される", () => {
			const { container } = render(<SignupFormComponent data-testid="signup-form" />);
			expect(container.firstChild).toHaveAttribute("data-testid", "signup-form");
		});
	});
});
