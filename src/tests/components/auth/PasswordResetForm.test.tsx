...
const TestWrapper = ({
	...
	return <PasswordResetForm pageState={pageState} form={form} onSubmit={onSubmit} />;
};

describe("PasswordResetForm", () => {
	test("loading状態でスケルトンが表示される", () => {
		const { container } = render(<TestWrapper pageState="loading" onSubmit={vi.fn()} />);
		...
		expect(screen.queryByRole("button", { name: "パスワードを更新" })).not.toBeInTheDocument();
	});
