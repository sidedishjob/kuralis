...
export default function BoardView({ maintenanceTasks }: BoardViewProps) {
  ...
  const tasksWithDueDate = maintenanceTasks.filter(
    (t) => t.nextDueDate !== null,
  );
      ...
      <div className="space-y-4">
        ...
        {tasksWithDueDate
          .filter(
            (t) =>
              t.nextDueDate !== null &&
              new Date(t.nextDueDate).setHours(0, 0, 0, 0) <
          ...
          .map((task) => (
            ...
            >
              ...
              <div className="text-xs text-kuralis-600 mt-1">
                {task.taskName}
              </div>
              <div className="text-xs text-accent-500 mt-2">
                ...
                <br />
                前回実施日:{" "}
                {format(new Date(task.lastPerformedAt), "yyyy/MM/dd")}
              </div>
            </Link>
          ))}
      </div>
