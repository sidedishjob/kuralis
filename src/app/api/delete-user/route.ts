...
export async function POST(req: NextRequest) {
	...
	try {
		...
		if (profileError) {
			throw new ApiError(500, `プロフィール削除エラー: ${profileError.message}`);
		}
		...
		if (authError) {
			throw new ApiError(500, `認証情報の削除に失敗しました: ${authError.message}`);
		}

		return NextResponse.json({ message: "アカウント削除が完了しました" }, { status: 200 });
	} catch (error: unknown) {
