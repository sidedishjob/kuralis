...
describe("FurnitureDetailBasicTab", () => {
	...
	describe("編集モード", () => {
		test("編集モードで入力フィールドが表示される", () => {
			render(
				<TestWrapper
					defaultValues={{ name: "サンプル家具", brand: "無印良品", location_id: 1 }}
				>
					<FurnitureDetailBasicTab
						furniture={mockFurniture}
						isEditing={true}
	...
	describe("データがない場合", () => {
		...
		test("カテゴリがundefinedの場合、空として表示される", () => {
			const furnitureWithoutCategory = { ...mockFurniture, category: undefined };
			render(
				<TestWrapper>
					<FurnitureDetailBasicTab
		...
		test("設置場所がundefinedの場合、空として表示される", () => {
			const furnitureWithoutLocation = { ...mockFurniture, location: undefined };
			render(
				<TestWrapper>
					<FurnitureDetailBasicTab
