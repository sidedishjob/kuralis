...
function buildSupabaseMock(tasks: TaskRow[], records: RecordRow[]) {
	const maintenanceTasksOrderMock = vi.fn().mockResolvedValue({ data: tasks, error: null });
	const maintenanceTasksEqMock = vi.fn().mockReturnValue({ order: maintenanceTasksOrderMock });
	const maintenanceTasksSelectMock = vi.fn().mockReturnValue({ eq: maintenanceTasksEqMock });

	const maintenanceRecordsOrderMock = vi.fn().mockResolvedValue({ data: records, error: null });
	const maintenanceRecordsInMock = vi
		.fn()
		.mockReturnValue({ order: maintenanceRecordsOrderMock });
	const maintenanceRecordsSelectMock = vi.fn().mockReturnValue({ in: maintenanceRecordsInMock });
